    ...
    <main className="site-shell min-h-screen">
      ...
      <section id="top" className="hero-section">
        <div className="hero-copy">
          <p className="eyebrow text-[#c5a059]">Designed &amp; Crafted in Pakistan</p>
          <h1>Objects with a past.<br /><em>Made for yours.</em></h1>
          <p className="hero-intro">A considered collection of clothing and carry goods, shaped by local hands and made to move with you.</p>
          <a href="#shop" className="bronze-button">Explore the collection <span>↓</span></a>
        </div>
        <div className="hero-stamp" aria-hidden="true"><span>LG</span><small>HANDMADE<br />IN PAKISTAN</small></div>
        <div className="flag-detail" aria-hidden="true" />
      </section>

      <section id="shop" className="shop-section">
        <div className="section-heading">
          <div><p className="eyebrow text-[#7c6232]">The first edition</p><h2>Made to be kept.</h2></div>
          <p className="max-w-sm text-sm leading-6 text-[#756b5e]">Small-batch pieces with honest materials, useful forms, and the marks of the hands that made them.</p>
        </div>
        <div className="filter-bar">
          <div className="category-tabs" role="tablist" aria-label="Product categories">
            {categories.map((category) => <button key={category} type="button" onClick={() => setActiveCategory(category)} className={activeCategory === category ? 'category-tab active' : 'category-tab'}>{category}</button>)}
          </div>
          <label className="search-field"><span>Search</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find a piece" /></label>
        </div>
        {loading ? <div className="catalog-message">Gathering the collection...</div> : filteredProducts.length === 0 ? <div className="catalog-message">No pieces match that search.</div> : (
          <div className="product-grid">
            {filteredProducts.map((product, index) => {
              const category = categoryFor(product);
              return <article key={product.id} className="product-card" style={{ animationDelay: `${index * 80}ms` }}>
                <div className={`product-art product-art-${category.toLowerCase().replace(' ', '-')}`}>
                  {product.image_url && <div className="product-photo" style={{ backgroundImage: `url(${product.image_url})` }} />}
                  <span>{product.title.slice(0, 2).toUpperCase()}</span><small>{category}</small>
                </div>
                <div className="product-info"><p className="eyebrow text-[#7c6232]">{category}</p><h3>{product.title}</h3><p className="product-description">{product.description}</p><div className="product-footer"><strong>{formatPrice(product.price)}</strong><button type="button" onClick={() => addToCart(product)} className="text-button">Buy now <span>↗</span></button></div></div>
              </article>;
            })}
          </div>
        )}
